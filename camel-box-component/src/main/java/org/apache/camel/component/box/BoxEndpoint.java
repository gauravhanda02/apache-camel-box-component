...
public class BoxEndpoint extends AbstractApiEndpoint<BoxApiName, BoxConfiguration> {
    ...
    private void createApiProxy(Map<String, Object> args) {
        if( args != null) {
            ...
            String authenticationType = (String) args.get("authenticationType");
            if (userId != null && userId.trim().length() >0 ) {
                this.configuration.setUserId(userId.trim());

            }
            if (authenticationType != null && authenticationType.trim().length() >0 ) {
                this.configuration.setAuthenticationType(authenticationType.trim());
            }
